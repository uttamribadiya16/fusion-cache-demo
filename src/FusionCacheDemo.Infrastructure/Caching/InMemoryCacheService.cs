...
public class InMemoryCacheService
{
    ...
    {
        _cache = cacheProvider.GetCache("InMemoryCache");
    ...
    public IEnumerable<DenormalizedZipCode> GetCachedDataOnlyAsync()
    {
        // Attempt to retrieve the data from the cache without fetching from the repository
        return _cache.GetOrDefault<IEnumerable<DenormalizedZipCode>>(
            "items:all",
            options => options
                .SetFailSafe(true, TimeSpan.FromHours(1))
        );
